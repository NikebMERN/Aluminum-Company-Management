...
export const updateSoldQuantity = async (req, res) => {
    ...
    try {
        ...
        // 1️⃣ Get item details (added price_per_item)
        const [itemRows] = await connection.query(
            "SELECT id, given_quantity, sold_quantity, sub_admin_id, shape, price_per_item FROM aluminum_items WHERE id = ?",
            [itemId]
        );
        ...
        res.json({
            ...
            quantitySold,
            remainingQuantity,
            pricePerItem: item.price_per_item // ✅ Added price_per_item in response
        });
    } catch (error) {
